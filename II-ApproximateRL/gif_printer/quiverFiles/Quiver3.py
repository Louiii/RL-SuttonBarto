...
def makeRandomXY(samples, w, h):
    X, Y = [], []
    for _ in range(samples * w):
        rowx, rowy = [], []
        for _ in range(samples * h):
            x, y = random.random(), random.random()
            while (x > 1 - 1/w and y > 1 - 1/h):
                x, y = random.random(), random.random()
            rowx.append(w*x)
            rowy.append(h*y)
        X.append(rowx)
...
def plotCts(costFn, w, h, **kwargs):
    ...
    _, ax1 = plt.subplots()
    # Make the arrows
    Q = ax1.quiver(X, Y, U, V, M, units='x', pivot='middle', width=0.01,
                    cmap="binary", scale=1/0.1)

    # Shade and label goal cell
    ax1.text(w-1+0.12, h-0.6, 'Goal', color="g", fontsize=15)#, transform=ax1.transAxes)
    fill([w-1,w,w,w-1], [h,h,h-1,h-1], 'g', alpha=0.4, edgecolor='g')
    ...
    plt.imshow(costFn(xs, ys), origin="lower", extent=[0, w, 0, h])#, cmap='PuRd')
    cb=plt.colorbar()
    cb.set_label("Costs")
    ...
    #     xs += 0.01
    #     ys += 0.005
    #     im = plt.imshow(costAll(xs, ys), animated=True)
    #     ims.append([im])
